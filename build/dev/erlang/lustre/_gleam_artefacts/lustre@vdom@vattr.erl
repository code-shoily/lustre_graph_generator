...
-type attribute(QPZ) :: {attribute, integer(), binary(), binary()} |
    {property, integer(), binary(), gleam@json:json()} |
    {event,
        integer(),
        binary(),
        gleam@dynamic@decode:decoder(handler(QPZ)),
        list(binary()),
        event_behaviour(),
        event_behaviour(),
        integer(),
...
-type handler(QQA) :: {handler, boolean(), boolean(), QQA}.
...
?DOC(false).
-spec merge(list(attribute(QQP)), list(attribute(QQP))) -> list(attribute(QQP)).
...
?DOC(false).
-spec compare(attribute(QQW), attribute(QQW)) -> gleam@order:order().
compare(A, B) ->
    gleam@string:compare(erlang:element(3, A), erlang:element(3, B)).

-file("src/lustre/vdom/vattr.gleam", 91).
?DOC(false).
-spec prepare(list(attribute(QQK))) -> list(attribute(QQK)).
prepare(Attributes) ->
    case Attributes of
        [] ->
            Attributes;
...
-spec event(
    binary(),
    gleam@dynamic@decode:decoder(handler(QQF)),
    list(binary()),
    event_behaviour(),
    event_behaviour(),
    integer(),
