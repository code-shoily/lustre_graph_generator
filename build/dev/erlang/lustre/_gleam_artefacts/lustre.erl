...
-type runtime(XFT) :: any() | {gleam_phantom, XFT}.
...
-spec application(
    fun((XGP) -> {XGQ, lustre@effect:effect(XGR)}),
    fun((XGQ, XGR) -> {XGQ, lustre@effect:effect(XGR)}),
    fun((XGQ) -> lustre@vdom@vnode:element(XGR))
) -> lustre@runtime@app:app(XGP, XGQ, XGR).
...
-spec element(lustre@vdom@vnode:element(XGC)) -> lustre@runtime@app:app(any(), nil, XGC).
element(View) ->
    application(
...
-spec simple(
    fun((XGI) -> XGJ),
    fun((XGJ, XGK) -> XGJ),
    fun((XGJ) -> lustre@vdom@vnode:element(XGK))
) -> lustre@runtime@app:app(XGI, XGJ, XGK).
...
-spec component(
    fun((XGY) -> {XGZ, lustre@effect:effect(XHA)}),
    fun((XGZ, XHA) -> {XGZ, lustre@effect:effect(XHA)}),
    fun((XGZ) -> lustre@vdom@vnode:element(XHA)),
    list(lustre@runtime@app:option(XHA))
) -> lustre@runtime@app:app(XGY, XGZ, XHA).
...
-spec named(
    lustre@runtime@app:app(XHJ, XHK, XHL),
    gleam@erlang@process:name(lustre@runtime@server@runtime:message(XHL))
) -> lustre@runtime@app:app(XHJ, XHK, XHL).
named(App, Name) ->
    {app,
        {some, Name},
...
-spec do_start(lustre@runtime@app:app(XID, any(), XIF), binary(), XID) -> {ok,
        runtime(XIF)} |
    {error, error()}.
do_start(_, _, _) ->
    {error, not_a_browser}.
...
-spec supervised(lustre@runtime@app:app(XIV, any(), XIX), XIV) -> gleam@otp@supervision:child_specification(gleam@erlang@process:subject(lustre@runtime@server@runtime:message(XIX))).
...
-spec factory(lustre@runtime@app:app(XJE, any(), XJG)) -> gleam@otp@factory_supervisor:builder(XJE, gleam@erlang@process:subject(lustre@runtime@server@runtime:message(XJG))).
factory(App) ->
...
-spec send(runtime(XJV), lustre@runtime@server@runtime:message(XJV)) -> nil.
...
-spec dispatch(XJY) -> lustre@runtime@server@runtime:message(XJY).
dispatch(Message) ->
...
-spec start(lustre@runtime@app:app(XHU, any(), XHW), binary(), XHU) -> {ok,
        runtime(XHW)} |
    {error, error()}.
...
-spec start_server_component(lustre@runtime@app:app(XIM, any(), XIO), XIM) -> {ok,
        runtime(XIO)} |
    {error, error()}.
