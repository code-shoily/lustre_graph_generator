...
-type diff(UUS) :: {diff,
        lustre@vdom@patch:patch(UUS),
        lustre@vdom@cache:cache(UUS)}.

-type partial_diff(UUT) :: {partial_diff,
        lustre@vdom@patch:patch(UUT),
        lustre@vdom@cache:cache(UUT),
        lustre@vdom@cache:events(UUT)}.

-type attribute_change(UUU) :: {attribute_change,
        list(lustre@vdom@vattr:attribute(UUU)),
        list(lustre@vdom@vattr:attribute(UUU)),
        lustre@vdom@cache:events(UUU)}.
...
-spec diff_attributes(
    boolean(),
    lustre@vdom@path:path(),
    lustre@vdom@cache:events(UVX),
    list(lustre@vdom@vattr:attribute(UVX)),
    list(lustre@vdom@vattr:attribute(UVX)),
    list(lustre@vdom@vattr:attribute(UVX)),
    list(lustre@vdom@vattr:attribute(UVX))
) -> attribute_change(UVX).
...
?DOC(false).
-spec do_diff(
    list(lustre@vdom@vnode:element(UVA)),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(UVA)),
    list(lustre@vdom@vnode:element(UVA)),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(UVA)),
    ...
    integer(),
    integer(),
    list(lustre@vdom@patch:change(UVA)),
    list(lustre@vdom@patch:patch(UVA)),
    lustre@vdom@path:path(),
    lustre@vdom@cache:cache(UVA),
    lustre@vdom@cache:events(UVA)
) -> partial_diff(UVA).
do_diff(
    Old,
    Old_keyed,
...
?DOC(false).
-spec diff(
    lustre@vdom@cache:cache(UUV),
    lustre@vdom@vnode:element(UUV),
    lustre@vdom@vnode:element(UUV)
) -> diff(UUV).
diff(Cache, Old, New) ->
