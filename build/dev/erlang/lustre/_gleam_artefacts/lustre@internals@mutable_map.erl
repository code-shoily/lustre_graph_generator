...
-type mutable_map(RVK, RVL) :: any() | {gleam_phantom, RVK, RVL}.
...
?DOC(false).
-spec unsafe_get(mutable_map(RVQ, RVR), RVQ) -> RVR.
unsafe_get(Map, Key) ->
    maps:get(Key, Map).

-file("src/lustre/internals/mutable_map.gleam", 34).
?DOC(false).
-spec get_or_compute(mutable_map(RVY, RVZ), RVY, fun(() -> RVZ)) -> RVZ.
...
?DOC(false).
-spec has_key(mutable_map(RWI, any()), RWI) -> boolean().
has_key(Map, Key) ->
    maps:is_key(Key, Map).

-file("src/lustre/internals/mutable_map.gleam", 61).
?DOC(false).
-spec insert(mutable_map(RWQ, RWR), RWQ, RWR) -> mutable_map(RWQ, RWR).
insert(Map, Key, Value) ->
    maps:put(Key, Value, Map).

-file("src/lustre/internals/mutable_map.gleam", 79).
?DOC(false).
-spec delete(mutable_map(RXC, RXD), RXC) -> mutable_map(RXC, RXD).
delete(Map, Key) ->
    maps:remove(Key, Map).
