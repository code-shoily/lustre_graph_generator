...
-opaque cache(UDY) :: {cache,
        events(UDY),
        lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(UDY)), lustre@vdom@vnode:element(UDY)),
        lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(UDY)), lustre@vdom@vnode:element(UDY)),
        list(binary()),
        list(binary())}.

-opaque events(UDZ) :: {events,
        lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UDZ))),
        lustre@internals@mutable_map:mutable_map(binary(), child(UDZ))}.

-type child(UEA) :: {child,
        fun((gleam@dynamic:dynamic_()) -> gleam@dynamic:dynamic_()),
        events(UEA)}.

-type added_children(UEB) :: {added_children,
        lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UEB))),
        lustre@internals@mutable_map:mutable_map(binary(), child(UEB)),
        lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(UEB)), lustre@vdom@vnode:element(UEB))}.

-opaque decoded_event(UEC) :: {decoded_event,
        binary(),
        lustre@vdom@vattr:handler(UEC)} |
...
?DOC(false).
-spec events(cache(UEN)) -> events(UEN).
events(Cache) ->
    erlang:element(2, Cache).

-file("src/lustre/vdom/cache.gleam", 106).
?DOC(false).
-spec update_events(cache(UEQ), events(UEQ)) -> cache(UEQ).
...
?DOC(false).
-spec memos(cache(UEU)) -> lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(UEU)), lustre@vdom@vnode:element(UEU)).
memos(Cache) ->
    erlang:element(3, Cache).
...
-spec get_old_memo(
    cache(UEX),
    fun(() -> lustre@vdom@vnode:element(UEX)),
    fun(() -> lustre@vdom@vnode:element(UEX))
) -> lustre@vdom@vnode:element(UEX).
...
-spec keep_memo(
    cache(UFC),
    fun(() -> lustre@vdom@vnode:element(UFC)),
    fun(() -> lustre@vdom@vnode:element(UFC))
) -> cache(UFC).
...
-spec add_memo(
    cache(UFH),
    fun(() -> lustre@vdom@vnode:element(UFH)),
    lustre@vdom@vnode:element(UFH)
) -> cache(UFH).
...
?DOC(false).
-spec do_add_event(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UFY))),
    lustre@vdom@path:path(),
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UFY))
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UFY))).
...
-spec add_event(
    ...
    lustre@vdom@path:path(),
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UFT))
) -> events(UFT).
...
-spec do_remove_event(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UGM))),
    lustre@vdom@path:path(),
    binary()
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UGM))).
...
?DOC(false).
-spec remove_event(events(UGJ), lustre@vdom@path:path(), binary()) -> events(UGJ).
...
?DOC(false).
-spec add_attributes(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UHB))),
    lustre@vdom@path:path(),
    list(lustre@vdom@vattr:attribute(UHB))
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UHB))).
...
-spec do_add_children(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UHT))),
    lustre@internals@mutable_map:mutable_map(binary(), child(UHT)),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(UHT)), lustre@vdom@vnode:element(UHT)),
    lustre@vdom@path:path(),
    integer(),
    list(lustre@vdom@vnode:element(UHT))
) -> added_children(UHT).
...
-spec add_children(
    ...
    lustre@vdom@path:path(),
    integer(),
    list(lustre@vdom@vnode:element(UHM))
) -> {cache(UHM), events(UHM)}.
...
-spec add_child(
    ...
    lustre@vdom@path:path(),
    integer(),
    lustre@vdom@vnode:element(UGV)
) -> {cache(UGV), events(UGV)}.
...
?DOC(false).
-spec from_node(lustre@vdom@vnode:element(UEH)) -> cache(UEH).
from_node(Root) ->
    Cache = new(),
...
-spec remove_attributes(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UIK))),
    lustre@vdom@path:path(),
    list(lustre@vdom@vattr:attribute(UIK))
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UIK))).
...
-spec do_remove_children(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(UIV))),
    lustre@internals@mutable_map:mutable_map(binary(), child(UIV)),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(UIV)), lustre@vdom@vnode:element(UIV)),
    lustre@vdom@path:path(),
    integer(),
    list(lustre@vdom@vnode:element(UIV))
) -> events(UIV).
...
-spec replace_child(
    ...
    lustre@vdom@path:path(),
    integer(),
    lustre@vdom@vnode:element(UJH),
    lustre@vdom@vnode:element(UJH)
) -> {cache(UJH), events(UJH)}.
...
?DOC(false).
-spec dispatch(cache(UJV), decoded_event(UJV)) -> {cache(UJV),
    {ok, lustre@vdom@vattr:handler(UJV)} | {error, nil}}.
...
?DOC(false).
-spec handle(cache(UJZ), binary(), binary(), gleam@dynamic:dynamic_()) -> {cache(UJZ),
    {ok, lustre@vdom@vattr:handler(UJZ)} | {error, nil}}.
