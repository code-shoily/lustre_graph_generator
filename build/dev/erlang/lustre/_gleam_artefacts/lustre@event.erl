...
-spec on(binary(), gleam@dynamic@decode:decoder(AANV)) -> lustre@vdom@vattr:attribute(AANV).
on(Name, Handler) ->
...
-spec advanced(
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(AANY))
) -> lustre@vdom@vattr:attribute(AANY).
...
-spec handler(AAOC, boolean(), boolean()) -> lustre@vdom@vattr:handler(AAOC).
...
-spec prevent_default(lustre@vdom@vattr:attribute(AAOE)) -> lustre@vdom@vattr:attribute(AAOE).
...
-spec stop_propagation(lustre@vdom@vattr:attribute(AAOH)) -> lustre@vdom@vattr:attribute(AAOH).
...
-spec debounce(lustre@vdom@vattr:attribute(AAOK), integer()) -> lustre@vdom@vattr:attribute(AAOK).
...
-spec throttle(lustre@vdom@vattr:attribute(AAON), integer()) -> lustre@vdom@vattr:attribute(AAON).
...
-spec on_keypress(fun((binary()) -> AAPE)) -> lustre@vdom@vattr:attribute(AAPE).
on_keypress(Msg) ->
    on(
        <<"keypress"/utf8>>,
        begin
...
-spec on_keydown(fun((binary()) -> AAPG)) -> lustre@vdom@vattr:attribute(AAPG).
on_keydown(Msg) ->
    on(
        <<"keydown"/utf8>>,
        begin
...
-spec on_keyup(fun((binary()) -> AAPI)) -> lustre@vdom@vattr:attribute(AAPI).
on_keyup(Msg) ->
    on(
        <<"keyup"/utf8>>,
        begin
...
-spec on_input(fun((binary()) -> AAPK)) -> lustre@vdom@vattr:attribute(AAPK).
on_input(Msg) ->
    on(
        <<"input"/utf8>>,
        begin
...
-spec on_change(fun((binary()) -> AAPM)) -> lustre@vdom@vattr:attribute(AAPM).
on_change(Msg) ->
    on(
        <<"change"/utf8>>,
        begin
...
-spec on_check(fun((boolean()) -> AAPO)) -> lustre@vdom@vattr:attribute(AAPO).
on_check(Msg) ->
    on(
        <<"change"/utf8>>,
        begin
...
-spec on_submit(fun((list({binary(), binary()})) -> AAPR)) -> lustre@vdom@vattr:attribute(AAPR).
on_submit(Msg) ->
    _pipe@2 = on(
        <<"submit"/utf8>>,
        begin
...
-spec on_focus(AAPV) -> lustre@vdom@vattr:attribute(AAPV).
on_focus(Msg) ->
    on(<<"focus"/utf8>>, gleam@dynamic@decode:success(Msg)).

-file("src/lustre/event.gleam", 352).
-spec on_blur(AAPX) -> lustre@vdom@vattr:attribute(AAPX).
on_blur(Msg) ->
