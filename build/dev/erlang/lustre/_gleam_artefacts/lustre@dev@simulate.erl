...
-opaque app(YDV, YDW, YDX) :: {app,
        fun((YDV) -> {YDW, lustre@effect:effect(YDX)}),
        fun((YDW, YDX) -> {YDW, lustre@effect:effect(YDX)}),
        fun((YDW) -> lustre@vdom@vnode:element(YDX))}.

-opaque simulation(YDY, YDZ) :: {simulation,
        fun((YDY, YDZ) -> {YDY, lustre@effect:effect(YDZ)}),
        fun((YDY) -> lustre@vdom@vnode:element(YDZ)),
        list(event(YDZ)),
        YDY,
        lustre@vdom@vnode:element(YDZ)}.

-type event(YEA) :: {dispatch, YEA} |
...
-spec simple(
    fun((YEB) -> YEC),
    fun((YEC, YED) -> YEC),
    fun((YEC) -> lustre@vdom@vnode:element(YED))
) -> app(YEB, YEC, YED).
...
-spec application(
    fun((YEI) -> {YEJ, lustre@effect:effect(YEK)}),
    fun((YEJ, YEK) -> {YEJ, lustre@effect:effect(YEK)}),
    fun((YEJ) -> lustre@vdom@vnode:element(YEK))
) -> app(YEI, YEJ, YEK).
...
-spec start(app(YER, YES, YET), YER) -> simulation(YES, YET).
start(App, Args) ->
...
-spec message(simulation(YEZ, YFA), YFA) -> simulation(YEZ, YFA).
...
-spec problem(simulation(YGF, YGG), binary(), binary()) -> simulation(YGF, YGG).
...
-spec model(simulation(YGL, any())) -> YGL.
model(Simulation) ->
    erlang:element(5, Simulation).
...
-spec view(simulation(any(), YGQ)) -> lustre@vdom@vnode:element(YGQ).
view(Simulation) ->
    erlang:element(6, Simulation).
...
-spec history(simulation(any(), YGV)) -> list(event(YGV)).
history(Simulation) ->
...
-spec click(simulation(YFM, YFN), lustre@dev@query:'query'()) -> simulation(YFM, YFN).
...
-spec input(simulation(YFS, YFT), lustre@dev@query:'query'(), binary()) -> simulation(YFS, YFT).
