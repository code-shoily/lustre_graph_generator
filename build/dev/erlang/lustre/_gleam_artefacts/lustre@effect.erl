...
-opaque effect(RPB) :: {effect,
        list(fun((actions(RPB)) -> nil)),
        list(fun((actions(RPB)) -> nil)),
        list(fun((actions(RPB)) -> nil))}.

-type actions(RPC) :: {actions,
        fun((RPC) -> nil),
        fun((binary(), gleam@json:json()) -> nil),
        fun((gleam@erlang@process:selector(RPC)) -> nil),
...
-spec do_comap_select(
    actions(RQM),
    gleam@erlang@process:selector(RQO),
    fun((RQO) -> RQM)
) -> nil.
...
-spec do_comap_actions(actions(RQI), fun((RQK) -> RQI)) -> actions(RQK).
...
-spec do_map(list(fun((actions(RQC)) -> nil)), fun((RQC) -> RQF)) -> list(fun((actions(RQF)) -> nil)).
do_map(Effects, F) ->
    gleam@list:map(
        Effects,
        fun(Effect) ->
...
-spec map(effect(RPY), fun((RPY) -> RQA)) -> effect(RQA).
map(Effect, F) ->
    {effect,
...
-spec perform(
    ...
    fun((RQQ) -> nil),
    fun((binary(), gleam@json:json()) -> nil),
    fun((gleam@erlang@process:selector(RQQ)) -> nil),
    fun(() -> gleam@dynamic:dynamic_()),
    fun((binary(), gleam@json:json()) -> nil)
) -> nil.
...
-spec from(fun((fun((RPF) -> nil)) -> nil)) -> effect(RPF).
from(Effect) ->
    Task = fun(Actions) ->
...
-spec before_paint(fun((fun((RPH) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(RPH).
before_paint(Effect) ->
    Task = fun(Actions) ->
...
-spec after_paint(fun((fun((RPJ) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(RPJ).
after_paint(Effect) ->
    Task = fun(Actions) ->
        Root = (erlang:element(5, Actions))(),
        Dispatch = erlang:element(2, Actions),
...
?DOC(false).
-spec select(
    fun((fun((RPN) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(RPN))
) -> effect(RPN).
select(Sel) ->
    Task = fun(Actions) ->
...
-spec batch(list(effect(RPU))) -> effect(RPU).
batch(Effects) ->
    gleam@list:fold(
        Effects,
        {effect, [], [], []},
