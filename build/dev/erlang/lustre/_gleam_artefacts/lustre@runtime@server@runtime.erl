...
-type state(VGO, VGP) :: {state,
        gleam@erlang@process:subject(message(VGP)),
        gleam@erlang@process:selector(message(VGP)),
        gleam@erlang@process:selector(message(VGP)),
        VGO,
        fun((VGO, VGP) -> {VGO, lustre@effect:effect(VGP)}),
        fun((VGO) -> lustre@vdom@vnode:element(VGP)),
        config(VGP),
        lustre@vdom@vnode:element(VGP),
        lustre@vdom@cache:cache(VGP),
        gleam@dict:dict(binary(), gleam@json:json()),
        gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(VGP)), gleam@erlang@process:monitor()),
        gleam@set:set(fun((lustre@runtime@transport:client_message(VGP)) -> nil))}.

-type config(VGQ) :: {config,
        boolean(),
        boolean(),
        gleam@dict:dict(binary(), fun((binary()) -> {ok, VGQ} | {error, nil})),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(VGQ)),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(VGQ)),
        gleam@option:option(VGQ),
        gleam@option:option(VGQ)}.

-type message(VGR) :: {client_dispatched_message,
        lustre@runtime@transport:server_message()} |
    {client_registered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(VGR))} |
    {client_deregistered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(VGR))} |
    {client_registered_callback,
        fun((lustre@runtime@transport:client_message(VGR)) -> nil)} |
    {client_deregistered_callback,
        fun((lustre@runtime@transport:client_message(VGR)) -> nil)} |
    {effect_added_selector, gleam@erlang@process:selector(message(VGR))} |
    {effect_dispatched_message, VGR} |
...
-spec handle_attribute_change(
    gleam@dict:dict(binary(), fun((binary()) -> {ok, VIA} | {error, nil})),
    binary(),
    binary()
) -> {ok, VIA} | {error, nil}.
...
-spec handle_property_change(
    gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(VIH)),
    binary(),
...
?DOC(false).
-spec handle_effect(
    gleam@erlang@process:subject(message(VIN)),
    lustre@effect:effect(VIN)
) -> nil.
...
?DOC(false).
-spec broadcast(
    gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(VIR)), gleam@erlang@process:monitor()),
    gleam@set:set(fun((lustre@runtime@transport:client_message(VIR)) -> nil)),
    lustre@runtime@transport:client_message(VIR)
) -> nil.
...
?DOC(false).
-spec loop(state(VHK, VHL), message(VHL)) -> gleam@otp@actor:next(state(VHK, VHL), message(VHL)).
loop(State, Message) ->
    case Message of
...
?DOC(false).
-spec start(
    gleam@option:option(gleam@erlang@process:name(message(VGV))),
    fun((VGZ) -> {VHA, lustre@effect:effect(VGV)}),
    fun((VHA, VGV) -> {VHA, lustre@effect:effect(VGV)}),
    fun((VHA) -> lustre@vdom@vnode:element(VGV)),
    config(VGV),
    VGZ
) -> {ok, gleam@otp@actor:started(gleam@erlang@process:subject(message(VGV)))} |
