...
-spec on_attribute_change(binary(), fun((binary()) -> {ok, WRW} | {error, nil})) -> lustre@runtime@app:option(WRW).
...
-spec on_property_change(binary(), gleam@dynamic@decode:decoder(WSA)) -> lustre@runtime@app:option(WSA).
...
-spec on_context_change(binary(), gleam@dynamic@decode:decoder(WSD)) -> lustre@runtime@app:option(WSD).
...
-spec on_form_autofill(fun((binary()) -> WSI)) -> lustre@runtime@app:option(WSI).
...
-spec on_form_reset(WSK) -> lustre@runtime@app:option(WSK).
...
-spec on_form_restore(fun((binary()) -> WSM)) -> lustre@runtime@app:option(WSM).
...
-spec on_connect(WSU) -> lustre@runtime@app:option(WSU).
on_connect(Message) ->
    {option,
        fun(Config) ->
            {config,
...
-spec on_adopt(WSW) -> lustre@runtime@app:option(WSW).
on_adopt(Message) ->
    {option,
        fun(Config) ->
            {config,
...
-spec on_disconnect(WSY) -> lustre@runtime@app:option(WSY).
...
-spec default_slot(
    list(lustre@vdom@vattr:attribute(WTA)),
    list(lustre@vdom@vnode:element(WTA))
) -> lustre@vdom@vnode:element(WTA).
...
-spec named_slot(
    binary(),
    list(lustre@vdom@vattr:attribute(WTG)),
    list(lustre@vdom@vnode:element(WTG))
...
-spec prerender(
    lustre@runtime@app:app(nil, any(), WUG),
    binary(),
    list(lustre@vdom@vattr:attribute(WUG)),
    list(lustre@vdom@vnode:element(WUG))
) -> lustre@vdom@vnode:element(WUG).
