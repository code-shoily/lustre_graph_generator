...
-type patch(TSX) :: {patch,
        integer(),
        integer(),
        list(change(TSX)),
        list(patch(TSX))}.

-type change(TSY) :: {replace_text, integer(), binary()} |
    {replace_inner_html, integer(), binary()} |
    {update,
        integer(),
        list(lustre@vdom@vattr:attribute(TSY)),
        list(lustre@vdom@vattr:attribute(TSY))} |
    {move, integer(), binary(), integer()} |
    {replace, integer(), integer(), lustre@vdom@vnode:element(TSY)} |
    {remove, integer(), integer()} |
    {insert, integer(), list(lustre@vdom@vnode:element(TSY)), integer()}.

-file("src/lustre/vdom/patch.gleam", 66).
?DOC(false).
-spec new(integer(), integer(), list(change(TSZ)), list(patch(TSZ))) -> patch(TSZ).
...
?DOC(false).
-spec add_child(patch(TUC), patch(TUC)) -> patch(TUC).
...
-spec replace_to_json(
    integer(),
    integer(),
    lustre@vdom@vnode:element(TWT),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(TWT)), lustre@vdom@vnode:element(TWT))
) -> gleam@json:json().
...
-spec insert_to_json(
    integer(),
    list(lustre@vdom@vnode:element(TWX)),
    integer(),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(TWX)), lustre@vdom@vnode:element(TWX))
) -> gleam@json:json().
...
-spec change_to_json(
    change(TUJ),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(TUJ)), lustre@vdom@vnode:element(TUJ))
) -> gleam@json:json().
...
-spec to_json(
    patch(TUG),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(TUG)), lustre@vdom@vnode:element(TUG))
) -> gleam@json:json().
to_json(Patch, Memos) ->
...
?DOC(false).
-spec replace(integer(), lustre@vdom@vnode:element(TTT)) -> change(TTT).
replace(Index, With) ->
    {replace, 5, Index, With}.

-file("src/lustre/vdom/patch.gleam", 116).
?DOC(false).
-spec insert(list(lustre@vdom@vnode:element(TTW)), integer()) -> change(TTW).
