...
-type message(PCG) :: {packet, bitstring()} | {user, PCG}.
...
-type connection(PCH) :: {connection,
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(glisten@internal@handler:message(PCH))}.

-opaque next(PCI, PCJ) :: {continue,
        PCI,
        gleam@option:option(gleam@erlang@process:selector(PCJ))} |
    normal_stop |
    {abnormal_stop, binary()}.

-opaque builder(PCK, PCL) :: {builder,
        glisten@socket@options:interface(),
        fun((connection(PCL)) -> {PCK,
            gleam@option:option(gleam@erlang@process:selector(PCL))}),
        fun((PCK, message(PCL), connection(PCL)) -> next(PCK, message(PCL))),
        gleam@option:option(fun((PCK) -> nil)),
        integer(),
        boolean(),
        boolean(),
...
-spec continue(PDL) -> next(PDL, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten.gleam", 151).
-spec with_selector(next(PDP, PDQ), gleam@erlang@process:selector(PDQ)) -> next(PDP, PDQ).
...
?DOC(false).
-spec convert_next(next(PEE, PEF)) -> glisten@internal@handler:next(PEE, PEF).
convert_next(Next) ->
    case Next of
...
?DOC(false).
-spec map_selector(next(PEK, PEL), fun((PEL) -> PEO)) -> next(PEK, PEO).
...
-spec map_user_selector(gleam@erlang@process:selector(message(PER))) -> gleam@erlang@process:selector(glisten@internal@handler:loop_message(PER)).
...
-spec convert_loop(
    fun((PEW, message(PEX), connection(PEX)) -> next(PEW, message(PEX)))
) -> fun((PEW, glisten@internal@handler:loop_message(PEX), glisten@internal@handler:connection(PEX)) -> glisten@internal@handler:next(PEW, glisten@internal@handler:loop_message(PEX))).
convert_loop(Loop) ->
    fun(Data, Msg, Conn) ->
        Conn@1 = {connection,
            erlang:element(3, Conn),
...
-spec convert_on_init(
    fun((connection(PFC)) -> {PFE,
        gleam@option:option(gleam@erlang@process:selector(PFC))})
) -> fun((glisten@internal@handler:connection(PFC)) -> {PFE,
    gleam@option:option(gleam@erlang@process:selector(PFC))}).
...
-spec new(
    fun((connection(PFK)) -> {PFM,
        gleam@option:option(gleam@erlang@process:selector(PFK))}),
    fun((PFM, message(PFK), connection(PFK)) -> next(PFM, message(PFK)))
) -> builder(PFM, PFK).
new(On_init, Loop) ->
...
-spec with_close(builder(PFT, PFU), fun((PFT) -> nil)) -> builder(PFT, PFU).
...
-spec with_pool_size(builder(PFZ, PGA), integer()) -> builder(PFZ, PGA).
...
?DOC(false).
-spec with_http2(builder(PGF, PGG)) -> builder(PGF, PGG).
with_http2(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_ipv6(builder(PGR, PGS)) -> builder(PGR, PGS).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(PGX, PGY), binary(), binary()) -> builder(PGX, PGY).
...
-spec bind(builder(PGL, PGM), binary()) -> builder(PGL, PGM).
