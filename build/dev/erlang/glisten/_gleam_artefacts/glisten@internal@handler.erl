...
-type message(OIF) :: {internal, internal_message()} | {user, OIF}.

-type loop_message(OIG) :: {packet, bitstring()} | {custom, OIG}.

-type loop_state(OIH, OII) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(OII)),
        glisten@transport:transport(),
        OIH}.

-type connection(OIJ) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(OIJ))}.

-type next(OIK, OIL) :: {continue,
        OIK,
        gleam@option:option(gleam@erlang@process:selector(OIL))} |
    normal_stop |
    {abnormal_stop, binary()}.

-type handler(OIM, OIN) :: {handler,
        glisten@socket:socket(),
        fun((OIM, loop_message(OIN), connection(OIN)) -> next(OIM, loop_message(OIN))),
        fun((connection(OIN)) -> {OIM,
            gleam@option:option(gleam@erlang@process:selector(OIN))}),
        gleam@option:option(fun((OIM) -> nil)),
        glisten@transport:transport()}.

-file("src/glisten/internal/handler.gleam", 65).
?DOC(false).
-spec continue(OJA) -> next(OJA, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten/internal/handler.gleam", 69).
?DOC(false).
-spec with_selector(next(OJE, OJF), gleam@erlang@process:selector(OJF)) -> next(OJE, OJF).
...
?DOC(false).
-spec start(handler(any(), OJU)) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(message(OJU)))} |
