...
-spec method_override(gleam@http@request:request(AIBS)) -> gleam@http@request:request(AIBS).
...
-spec sort_keys(list({binary(), AIDJ})) -> list({binary(), AIDJ}).
sort_keys(Pairs) ->
    gleam@list:sort(
        Pairs,
        fun(A, B) ->
...
-spec or_500({ok, AICD} | {error, any()}) -> {ok, AICD} |
    {error, gleam@http@response:response(body())}.
or_500(Result) ->
    case Result of
        {ok, Value} ->
...
-spec fn_with_bad_request_error(
    fun((AICR) -> {ok, AICS} | {error, any()}),
    binary()
) -> fun((AICR) -> {ok, AICS} | {error, gleam@http@response:response(body())}).
...
-spec multipart_body(
    ...
    integer(),
    integer(),
    fun((AICL, bitstring()) -> {ok, AICL} |
        {error, gleam@http@response:response(body())}),
    AICL
) -> {ok, {gleam@option:option(buffered_reader()), integer(), AICL}} |
    {error, gleam@http@response:response(body())}.
