...
-type action(IJX) :: stop | {continue, IJX, fun(() -> action(IJX))}.

-opaque yielder(IJY) :: {yielder, fun(() -> action(IJY))}.

-type step(IJZ, IKA) :: {next, IJZ, IKA} | done.

-type chunk(IKB, IKC) :: {another_by,
        list(IKB),
        IKC,
        IKB,
        fun(() -> action(IKB))} |
...
-type sized_chunk(IKD) :: {another, list(IKD), fun(() -> action(IKD))} |
    {last, list(IKD)} |
    no_more.
...
-spec unfold_loop(IKL, fun((IKL) -> step(IKM, IKL))) -> fun(() -> action(IKM)).
...
-spec unfold(IKG, fun((IKG) -> step(IKH, IKG))) -> yielder(IKH).
unfold(Initial, F) ->
    _pipe = Initial,
...
-spec repeatedly(fun(() -> IKQ)) -> yielder(IKQ).
repeatedly(F) ->
...
-spec from_list(list(IKU)) -> yielder(IKU).
from_list(List) ->
...
-spec transform(yielder(ILE), ILG, fun((ILG, ILE) -> step(ILH, ILG))) -> yielder(ILH).
transform(Yielder, Initial, F) ->
    _pipe = transform_loop(erlang:element(2, Yielder), Initial, F),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 204).
-spec fold_loop(fun(() -> action(ILO)), fun((ILQ, ILO) -> ILQ), ILQ) -> ILQ.
...
-spec fold(yielder(ILL), ILN, fun((ILN, ILL) -> ILN)) -> ILN.
...
-spec to_list(yielder(ILT)) -> list(ILT).
to_list(Yielder) ->
    _pipe = Yielder,
...
-spec step(yielder(ILW)) -> step(ILW, yielder(ILW)).
step(Yielder) ->
...
-spec take_loop(fun(() -> action(IME)), integer()) -> fun(() -> action(IME)).
...
-spec take(yielder(IMB), integer()) -> yielder(IMB).
...
-spec drop_loop(fun(() -> action(IMK)), integer()) -> action(IMK).
...
-spec map(yielder(IMN), fun((IMN) -> IMP)) -> yielder(IMP).
map(Yielder, F) ->
    _pipe = erlang:element(2, Yielder),
...
-spec map2(yielder(IMV), yielder(IMX), fun((IMV, IMX) -> IMZ)) -> yielder(IMZ).
...
-spec append_loop(fun(() -> action(INL)), fun(() -> action(INL))) -> action(INL).
...
-spec append(yielder(INH), yielder(INH)) -> yielder(INH).
append(First, Second) ->
    _pipe = fun() ->
...
-spec flatten_loop(fun(() -> action(yielder(INT)))) -> action(INT).
...
-spec flatten(yielder(yielder(INP))) -> yielder(INP).
flatten(Yielder) ->
...
-spec concat(list(yielder(INX))) -> yielder(INX).
concat(Yielders) ->
...
-spec flat_map(yielder(IOB), fun((IOB) -> yielder(IOD))) -> yielder(IOD).
flat_map(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    flatten(_pipe@1).
...
-spec filter_loop(fun(() -> action(IOJ)), fun((IOJ) -> boolean())) -> action(IOJ).
...
-spec filter(yielder(IOG), fun((IOG) -> boolean())) -> yielder(IOG).
...
-spec filter_map(yielder(IOM), fun((IOM) -> {ok, IOO} | {error, any()})) -> yielder(IOO).
...
-spec cycle(yielder(IPA)) -> yielder(IPA).
cycle(Yielder) ->
    _pipe = repeat(Yielder),
    flatten(_pipe).

-file("src/gleam/yielder.gleam", 709).
-spec find_loop(fun(() -> action(IPI)), fun((IPI) -> boolean())) -> {ok, IPI} |
    {error, nil}.
...
-spec find(yielder(IPE), fun((IPE) -> boolean())) -> {ok, IPE} | {error, nil}.
...
-spec find_map(yielder(IPM), fun((IPM) -> {ok, IPO} | {error, any()})) -> {ok,
        IPO} |
    {error, nil}.
...
-spec index_loop(fun(() -> action(IQF)), integer()) -> fun(() -> action({IQF,
    integer()})).
...
-spec index(yielder(IQC)) -> yielder({IQC, integer()}).
index(Yielder) ->
    _pipe = erlang:element(2, Yielder),
...
-spec iterate(IQI, fun((IQI) -> IQI)) -> yielder(IQI).
iterate(Initial, F) ->
    unfold(Initial, fun(Element) -> {next, Element, F(Element)} end).

-file("src/gleam/yielder.gleam", 832).
-spec take_while_loop(fun(() -> action(IQN)), fun((IQN) -> boolean())) -> fun(() -> action(IQN)).
...
-spec take_while(yielder(IQK), fun((IQK) -> boolean())) -> yielder(IQK).
...
-spec drop_while_loop(fun(() -> action(IQT)), fun((IQT) -> boolean())) -> action(IQT).
...
-spec drop_while(yielder(IQQ), fun((IQQ) -> boolean())) -> yielder(IQQ).
...
-spec scan_loop(fun(() -> action(IRA)), fun((IRC, IRA) -> IRC), IRC) -> fun(() -> action(IRC)).
...
-spec scan(yielder(IQW), IQY, fun((IQY, IQW) -> IQY)) -> yielder(IQY).
...
-spec zip_loop(fun(() -> action(IRJ)), fun(() -> action(IRL))) -> fun(() -> action({IRJ,
    IRL})).
zip_loop(Left, Right) ->
    fun() -> case Left() of
            stop ->
...
-spec zip(yielder(IRE), yielder(IRG)) -> yielder({IRE, IRG}).
zip(Left, Right) ->
    _pipe = zip_loop(erlang:element(2, Left), erlang:element(2, Right)),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 1000).
-spec next_chunk(fun(() -> action(IRY)), fun((IRY) -> ISA), ISA, list(IRY)) -> chunk(IRY, ISA).
...
-spec chunk_loop(fun(() -> action(IRT)), fun((IRT) -> IRV), IRV, IRT) -> action(list(IRT)).
...
-spec chunk(yielder(IRO), fun((IRO) -> any())) -> yielder(list(IRO)).
chunk(Yielder, F) ->
...
-spec next_sized_chunk(fun(() -> action(ISM)), integer(), list(ISM)) -> sized_chunk(ISM).
...
-spec sized_chunk_loop(fun(() -> action(ISI)), integer()) -> fun(() -> action(list(ISI))).
...
-spec sized_chunk(yielder(ISE), integer()) -> yielder(list(ISE)).
...
-spec intersperse_loop(fun(() -> action(IST)), IST) -> action(IST).
...
-spec intersperse(yielder(ISQ), ISQ) -> yielder(ISQ).
...
-spec any_loop(fun(() -> action(ISY)), fun((ISY) -> boolean())) -> boolean().
...
-spec any(yielder(ISW), fun((ISW) -> boolean())) -> boolean().
any(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    any_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1228).
-spec all_loop(fun(() -> action(ITC)), fun((ITC) -> boolean())) -> boolean().
...
-spec all(yielder(ITA), fun((ITA) -> boolean())) -> boolean().
all(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    all_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1273).
-spec update_group_with(ITS) -> fun((gleam@option:option(list(ITS))) -> list(ITS)).
...
-spec group_updater(fun((ITK) -> ITL)) -> fun((gleam@dict:dict(ITL, list(ITK)), ITK) -> gleam@dict:dict(ITL, list(ITK))).
group_updater(F) ->
...
-spec group(yielder(ITE), fun((ITE) -> ITG)) -> gleam@dict:dict(ITG, list(ITE)).
group(Yielder, Key) ->
    _pipe = Yielder,
...
-spec reduce(yielder(ITW), fun((ITW, ITW) -> ITW)) -> {ok, ITW} | {error, nil}.
reduce(Yielder, F) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec last(yielder(IUA)) -> {ok, IUA} | {error, nil}.
last(Yielder) ->
    _pipe = Yielder,
...
-spec single(IUI) -> yielder(IUI).
single(Elem) ->
    once(fun() -> Elem end).

-file("src/gleam/yielder.gleam", 1402).
-spec interleave_loop(fun(() -> action(IUO)), fun(() -> action(IUO))) -> action(IUO).
...
-spec interleave(yielder(IUK), yielder(IUK)) -> yielder(IUK).
...
-spec fold_until_loop(
    fun(() -> action(IUW)),
    fun((IUY, IUW) -> gleam@list:continue_or_stop(IUY)),
    IUY
) -> IUY.
...
-spec fold_until(
    yielder(IUS),
    IUU,
    fun((IUU, IUS) -> gleam@list:continue_or_stop(IUU))
) -> IUU.
...
-spec try_fold(yielder(IVA), IVC, fun((IVC, IVA) -> {ok, IVC} | {error, IVD})) -> {ok,
        IVC} |
    {error, IVD}.
...
-spec first(yielder(IVQ)) -> {ok, IVQ} | {error, nil}.
first(Yielder) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec at(yielder(IVU), integer()) -> {ok, IVU} | {error, nil}.
at(Yielder, Index) ->
    _pipe = Yielder,
...
-spec each(yielder(IWC), fun((IWC) -> any())) -> nil.
each(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    run(_pipe@1).
...
-spec yield(IWF, fun(() -> yielder(IWF))) -> yielder(IWF).
yield(Element, Next) ->
    {yielder,
        fun() ->
...
-spec prepend(yielder(IWI), IWI) -> yielder(IWI).
