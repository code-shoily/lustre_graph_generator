...
-opaque next(ADQX, ADQY) :: {continue,
        ADQX,
        gleam@option:option(gleam@erlang@process:selector(ADQY))} |
    normal_stop |
...
-opaque builder(ADQZ, ADRA) :: {builder,
        integer(),
        fun((gleam@http@request:request(ADQZ)) -> gleam@http@response:response(ADRA)),
...
-type websocket_message(ADRB) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, ADRB}.
...
-spec continue(ADRC) -> next(ADRC, any()).
continue(State) ->
    {continue, State, none}.

-file("src/mist.gleam", 58).
-spec with_selector(next(ADRG, ADRH), gleam@erlang@process:selector(ADRH)) -> next(ADRG, ADRH).
...
-spec convert_next(next(ADRV, ADRW)) -> mist@internal@next:next(ADRV, ADRW).
convert_next(Next) ->
    case Next of
...
-spec new(
    fun((gleam@http@request:request(ADSX)) -> gleam@http@response:response(ADSZ))
) -> builder(ADSX, ADSZ).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(ADTD, ADTE), integer()) -> builder(ADTD, ADTE).
port(Builder, Port) ->
    {builder,
        Port,
...
-spec read_request_body(
    builder(bitstring(), ADTJ),
    integer(),
    gleam@http@response:response(ADTJ)
) -> builder(mist@internal@http:connection(), ADTJ).
...
-spec bind(builder(ADTV, ADTW), binary()) -> builder(ADTV, ADTW).
...
-spec with_ipv6(builder(ADUB, ADUC)) -> builder(ADUB, ADUC).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(ADUH, ADUI), binary(), binary()) -> builder(ADUH, ADUI).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(ADUX)
) -> {ok, websocket_message(ADUX)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((ADVD, websocket_message(ADVE), mist@internal@websocket:websocket_connection()) -> next(ADVD, ADVE)),
    fun((mist@internal@websocket:websocket_connection()) -> {ADVD,
        gleam@option:option(gleam@erlang@process:selector(ADVE))}),
    fun((ADVD) -> nil)
...
-spec server_sent_events(
    ...
    fun((gleam@erlang@process:subject(ADVS)) -> {ok,
            gleam@otp@actor:initialised(ADVU, ADVS, any())} |
        {error, binary()}),
    fun((ADVU, ADVS, s_s_e_connection()) -> gleam@otp@actor:next(ADVU, ADVS))
) -> gleam@http@response:response(response_data()).
