...
-type valid_message(ADCN) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, ADCN}.

-type websocket_message(ADCO) :: {valid, valid_message(ADCO)} | invalid.
...
-type handler_message(ADCP) :: {internal, gramps@websocket:frame()} |
    {user, ADCP}.

-type websocket_state(ADCQ) :: {websocket_state,
        bitstring(),
        ADCQ,
...
-spec map_user_selector(
    gleam@option:option(gleam@erlang@process:selector(ADDW))
) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(ADDW))).
...
-spec apply_frames(
    ...
    fun((ADDM, handler_message(ADDN), websocket_connection()) -> mist@internal@next:next(ADDM, ADDN)),
    websocket_connection(),
    mist@internal@next:next(ADDM, websocket_message(ADDN)),
    fun((ADDM) -> nil)
) -> mist@internal@next:next(ADDM, websocket_message(ADDN)).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {ADCZ,
        gleam@option:option(gleam@erlang@process:selector(ADDA))}),
    fun((ADCZ) -> nil),
    fun((ADCZ, handler_message(ADDA), websocket_connection()) -> mist@internal@next:next(ADCZ, ADDA)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(websocket_message(ADDA)))} |
    {error, nil}.
