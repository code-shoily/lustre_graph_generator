...
-type tree(LEC) :: empty | {tree, LEC, list(tree(LEC))}.

-opaque heap(LED) :: {heap, tree(LED), fun((LED, LED) -> gleam@order:order())}.

-file("src/gleamy/pairing_heap.gleam", 18).
?DOC(" Creates a new empty heap with the provided comparison function.\n").
-spec new(fun((LEE, LEE) -> gleam@order:order())) -> heap(LEE).
new(Compare) ->
    {heap, empty, Compare}.
...
-spec find_min(heap(LEJ)) -> {ok, LEJ} | {error, nil}.
find_min(Heap) ->
...
-spec merge_trees(tree(LEW), tree(LEW), fun((LEW, LEW) -> gleam@order:order())) -> tree(LEW).
...
-spec insert(heap(LEG), LEG) -> heap(LEG).
insert(Heap, Key) ->
    {heap,
        merge_trees(
            {tree, Key, []},
...
-spec merge(heap(LES), heap(LES)) -> heap(LES).
merge(Heap1, Heap2) ->
...
-spec merge_pairs(list(tree(LFA)), fun((LFA, LFA) -> gleam@order:order())) -> tree(LFA).
...
-spec delete_min(heap(LEN)) -> {ok, {LEN, heap(LEN)}} | {error, nil}.
delete_min(Heap) ->
    case erlang:element(2, Heap) of
        {tree, X, Xs} ->
            {ok,
