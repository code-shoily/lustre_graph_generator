...
-spec new(fun((KVU, KVU) -> gleam@order:order())) -> gleamy@red_black_tree_map:map_(KVU, any()).
new(Compare) ->
...
-spec insert(gleamy@red_black_tree_map:map_(KVY, KVZ), KVY, KVZ) -> gleamy@red_black_tree_map:map_(KVY, KVZ).
...
-spec get(gleamy@red_black_tree_map:map_(KWE, KWF), KWE) -> {ok, KWF} |
    {error, nil}.
get(Map, Key) ->
...
-spec has_key(gleamy@red_black_tree_map:map_(KWK, any()), KWK) -> boolean().
has_key(Map, Key) ->
...
-spec delete(gleamy@red_black_tree_map:map_(KWO, KWP), KWO) -> gleamy@red_black_tree_map:map_(KWO, KWP).
delete(Map, Key) ->
...
-spec fold(
    gleamy@red_black_tree_map:map_(KWY, KWZ),
    KXC,
    fun((KXC, KWY, KWZ) -> KXC)
) -> KXC.
...
-spec filter(
    gleamy@red_black_tree_map:map_(KXD, KXE),
    fun((KXD, KXE) -> boolean())
) -> gleamy@red_black_tree_map:map_(KXD, KXE).
filter(Map, Property) ->
...
-spec merge(
    gleamy@red_black_tree_map:map_(KXJ, KXK),
    gleamy@red_black_tree_map:map_(KXJ, KXK)
) -> gleamy@red_black_tree_map:map_(KXJ, KXK).
...
-spec take(gleamy@red_black_tree_map:map_(KXR, KXS), list(KXR)) -> gleamy@red_black_tree_map:map_(KXR, KXS).
take(Map, Desired) ->
    case Desired of
        [X | Xs] ->
...
-spec from_list(list({KXY, KXZ}), fun((KXY, KXY) -> gleam@order:order())) -> gleamy@red_black_tree_map:map_(KXY, KXZ).
...
-spec to_list(gleamy@red_black_tree_map:map_(KYD, KYE)) -> list({KYD, KYE}).
to_list(Map) ->
