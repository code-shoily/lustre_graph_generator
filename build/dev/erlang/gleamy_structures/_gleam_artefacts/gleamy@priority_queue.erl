...
-spec new(fun((LHG, LHG) -> gleam@order:order())) -> gleamy@pairing_heap:heap(LHG).
new(Compare) ->
...
-spec pop(gleamy@pairing_heap:heap(LHI)) -> {ok,
        {LHI, gleamy@pairing_heap:heap(LHI)}} |
    {error, nil}.
pop(Queue) ->
...
-spec peek(gleamy@pairing_heap:heap(LHN)) -> {ok, LHN} | {error, nil}.
peek(Queue) ->
...
-spec push(gleamy@pairing_heap:heap(LHR), LHR) -> gleamy@pairing_heap:heap(LHR).
push(Queue, Item) ->
...
-spec reorder(
    gleamy@pairing_heap:heap(LHY),
    fun((LHY, LHY) -> gleam@order:order())
) -> gleamy@pairing_heap:heap(LHY).
...
-spec from_list(list(LIB), fun((LIB, LIB) -> gleam@order:order())) -> gleamy@pairing_heap:heap(LIB).
...
-spec to_list(gleamy@pairing_heap:heap(LIE)) -> list(LIE).
to_list(Queue) ->
    case gleamy@pairing_heap:delete_min(Queue) of
        {ok, {X, Q}} ->
            [X | to_list(Q)];
