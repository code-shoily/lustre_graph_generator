...
-opaque glint(MOD) :: {glint, config(), command_node(MOD)}.

-type args_count() :: {eq_args, integer()} | {min_args, integer()}.

-opaque command(MOE) :: {command,
        fun((named_args(), list(binary()), flags()) -> MOE),
        flags(),
        binary(),
        gleam@option:option(args_count()),
        list(binary())}.

-type internal_command(MOF) :: {internal_command,
        fun((named_args(), list(binary()), flags()) -> MOF),
        flags(),
        gleam@option:option(args_count()),
        list(binary())}.
...
-type command_node(MOG) :: {command_node,
        gleam@option:option(internal_command(MOG)),
        gleam@dict:dict(binary(), command_node(MOG)),
        flags(),
        binary()}.

-type out(MOH) :: {out, MOH} | {help, binary()}.
...
-opaque flag(MOI) :: {flag,
        binary(),
        binary(),
        fun((binary()) -> {ok, MOI} | {error, snag:snag()}),
        fun((flag_internals(MOI)) -> value()),
        fun((flags(), binary()) -> {ok, MOI} | {error, snag:snag()}),
        gleam@option:option(MOI)}.

-type flag_internals(MOJ) :: {flag_internals,
        gleam@option:option(MOJ),
        fun((binary()) -> {ok, MOJ} | {error, snag:snag()})}.
...
-spec pretty_help(glint(MOQ), pretty_help()) -> glint(MOQ).
...
-spec with_name(glint(MOT), binary()) -> glint(MOT).
...
-spec without_exit(glint(MOW)) -> glint(MOW).
without_exit(Glint) ->
    {glint,
        begin
            _record = erlang:element(2, Glint),
...
-spec as_module(glint(MOZ)) -> glint(MOZ).
as_module(Glint) ->
    {glint,
        begin
            _record = erlang:element(2, Glint),
...
-spec with_indent_width(glint(MPC), integer()) -> glint(MPC).
...
-spec with_max_output_width(glint(MPF), integer()) -> glint(MPF).
...
-spec with_min_first_column_width(glint(MPI), integer()) -> glint(MPI).
...
-spec with_column_gap(glint(MPL), integer()) -> glint(MPL).
...
-spec global_help(glint(MPU), binary()) -> glint(MPU).
...
-spec map_command(command(MQJ), fun((MQJ) -> MQL)) -> command(MQL).
...
-spec command_help(binary(), fun(() -> command(MQN))) -> command(MQN).
command_help(Desc, F) ->
    _record = F(),
    {command,
        erlang:element(2, _record),
...
-spec unnamed_args(args_count(), fun(() -> command(MQQ))) -> command(MQQ).
...
-spec named_arg(
    binary(),
    fun((fun((named_args()) -> binary())) -> command(MQT))
) -> command(MQT).
named_arg(Name, F) ->
    Cmd = begin
        F(
...
-spec new_builder(
    binary(),
    fun((flag_internals(MTH)) -> value()),
    fun((flags(), binary()) -> {ok, MTH} | {error, snag:snag()}),
    fun((binary()) -> {ok, MTH} | {error, snag:snag()})
) -> flag(MTH).
...
-spec wrap_with_constraint(
    fun((binary()) -> {ok, MTT} | {error, snag:snag()}),
    fun((MTT) -> {ok, MTT} | {error, snag:snag()})
) -> fun((binary()) -> {ok, MTT} | {error, snag:snag()}).
...
-spec flag_constraint(flag(MTP), fun((MTP) -> {ok, MTP} | {error, snag:snag()})) -> flag(MTP).
...
-spec flag_help(flag(MUI), binary()) -> flag(MUI).
...
-spec flag_default(flag(MUL), MUL) -> flag(MUL).
...
-spec flag(
    flag(MQW),
    fun((fun((flags()) -> {ok, MQW} | {error, snag:snag()})) -> command(MQZ))
) -> command(MQZ).
flag(Flag, F) ->
    Cmd = F(
        fun(_capture) ->
...
-spec fold(flags(), MUO, fun((MUO, binary(), flag_entry()) -> MUO)) -> MUO.
fold(Flags, Acc, F) ->
...
-spec command(fun((named_args(), list(binary()), flags()) -> MQG)) -> command(MQG).
command(Runner) ->
...
-spec get_value(
    flags(),
    binary(),
    fun((flag_entry()) -> {ok, MVB} | {error, snag:snag()})
) -> {ok, MVB} | {error, snag:snag()}.
...
-spec get_flag(flags(), flag(MVE)) -> {ok, MVE} | {error, snag:snag()}.
get_flag(Flags, Flag) ->
...
-spec do_update_at(
    command_node(MVX),
    list(binary()),
    fun((command_node(MVX)) -> command_node(MVX))
) -> command_node(MVX).
...
-spec update_at(
    glint(MVR),
    list(binary()),
    fun((command_node(MVR)) -> command_node(MVR))
) -> glint(MVR).
...
-spec path_help(glint(MPQ), list(binary()), binary()) -> glint(MPQ).
...
-spec add(glint(MPX), list(binary()), command(MPX)) -> glint(MPX).
...
-spec group_flag(glint(MRC), list(binary()), flag(any())) -> glint(MRC).
...
?DOC(false).
-spec execute(glint(MRI), list(binary())) -> {ok, out(MRI)} | {error, binary()}.
execute(Glint, Args) ->
    Help_flag = <<"--"/utf8,
        (erlang:element(
            2,
...
-spec run_and_handle(glint(MSH), list(binary()), fun((MSH) -> any())) -> nil.
