...
-spec one_of(list(MHT)) -> fun((MHT) -> {ok, MHT} | {error, snag:snag()}).
one_of(Allowed) ->
...
-spec none_of(list(MHW)) -> fun((MHW) -> {ok, MHW} | {error, snag:snag()}).
none_of(Disallowed) ->
...
-spec each(fun((MHZ) -> {ok, MHZ} | {error, snag:snag()})) -> fun((list(MHZ)) -> {ok,
        list(MHZ)} |
    {error, snag:snag()}).
each(Constraint) ->
