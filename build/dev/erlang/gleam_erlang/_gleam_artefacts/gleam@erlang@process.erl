...
-opaque subject(EWE) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(EWE)}.

-type name(EWF) :: any() | {gleam_phantom, EWF}.

-type do_not_leak() :: any().

-type selector(EWG) :: any() | {gleam_phantom, EWG}.
...
-spec 'receive'(subject(EXE), integer()) -> {ok, EXE} | {error, nil}.
...
-spec receive_forever(subject(EXM)) -> EXM.
...
-spec selector_receive(selector(EXQ), integer()) -> {ok, EXQ} | {error, nil}.
...
-spec selector_receive_forever(selector(EXU)) -> EXU.
...
-spec map_selector(selector(EXW), fun((EXW) -> EXY)) -> selector(EXY).
map_selector(A, B) ->
...
-spec merge_selector(selector(EYA), selector(EYA)) -> selector(EYA).
merge_selector(A, B) ->
...
-spec select_map(selector(EYL), subject(EYN), fun((EYN) -> EYL)) -> selector(EYL).
...
-spec select(selector(EYH), subject(EYH)) -> selector(EYH).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> EYV)
) -> selector(EYV).
...
-spec select_other(selector(EYZ), fun((gleam@dynamic:dynamic_()) -> EYZ)) -> selector(EYZ).
...
-spec deselect(selector(EYQ), subject(any())) -> selector(EYQ).
...
-spec select_specific_monitor(selector(EZL), monitor(), fun((down()) -> EZL)) -> selector(EZL).
...
-spec select_monitors(selector(EZO), fun((down()) -> EZO)) -> selector(EZO).
...
-spec select_trapped_exits(selector(EYE), fun((exit_message()) -> EYE)) -> selector(EYE).
...
-spec deselect_specific_monitor(selector(EZR), monitor()) -> selector(EZR).
...
-spec send_after(subject(FAN), integer(), FAN) -> timer().
...
-spec perform_call(
    subject(EZU),
    fun((subject(EZW)) -> EZU),
    fun((selector(EZW)) -> {ok, EZW} | {error, nil})
) -> EZW.
...
-spec call(subject(FAB), integer(), fun((subject(FAD)) -> FAB)) -> FAD.
...
-spec call_forever(subject(FAF), fun((subject(FAH)) -> FAF)) -> FAH.
