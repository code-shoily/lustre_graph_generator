...
-spec be_ok({ok, MDC} | {error, any()}) -> MDC.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, MDH}) -> MDH.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(MDK)) -> MDK.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
