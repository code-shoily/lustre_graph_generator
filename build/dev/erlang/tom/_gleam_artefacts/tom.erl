...
-spec push_key({ok, AFEX} | {error, get_error()}, binary()) -> {ok, AFEX} |
    {error, get_error()}.
push_key(Result, Key) ->
    case Result of
        {ok, T} ->
...
-spec expect_end_of_line(
    list(binary()),
    fun((list(binary())) -> {ok, {AFHC, list(binary())}} |
        {error, parse_error()})
) -> {ok, {AFHC, list(binary())}} | {error, parse_error()}.
...
-spec do(
    {ok, {AFHN, list(binary())}} | {error, parse_error()},
    fun((AFHN, list(binary())) -> {ok, AFHQ} | {error, parse_error()})
) -> {ok, AFHQ} | {error, parse_error()}.
do(Result, Next) ->
    case Result of
        {ok, {A, Input}} ->
            Next(A, Input);
...
-spec expect(
    list(binary()),
    binary(),
    fun((list(binary())) -> {ok, {AFHV, list(binary())}} |
        {error, parse_error()})
) -> {ok, {AFHV, list(binary())}} | {error, parse_error()}.
