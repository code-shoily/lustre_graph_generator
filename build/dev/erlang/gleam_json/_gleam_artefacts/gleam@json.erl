...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(EDV)) -> {ok, EDV} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(EDP)) -> {ok, EDP} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(EDL)) -> {ok, EDL} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(EEB), fun((EEB) -> json())) -> json().
...
-spec array(list(EEF), fun((EEF) -> json())) -> json().
