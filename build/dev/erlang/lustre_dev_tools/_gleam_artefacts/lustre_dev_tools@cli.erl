...
-spec string(
    ...
    fun((fun((glint:flags()) -> {ok, binary()} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AJND))
) -> glint:command(AJND).
...
-spec flag(
    ...
    list(binary()),
    lustre_dev_tools@project:project(),
    fun((binary()) -> glint:flag(AJNU)),
    fun((gleam@dict:dict(binary(), tom:toml()), list(binary())) -> {ok, AJNU} |
        {error, tom:get_error()}),
    binary(),
    fun((fun((glint:flags()) -> {ok, AJNU} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AJOD))
) -> glint:command(AJOD).
...
-spec bool(
    ...
    fun((fun((glint:flags()) -> {ok, boolean()} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AJMX))
) -> glint:command(AJMX).
...
-spec string_list(
    ...
    fun((fun((glint:flags()) -> {ok, list(binary())} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AJNK))
) -> glint:command(AJNK).
...
-spec int(
    ...
    fun((fun((glint:flags()) -> {ok, integer()} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AJNQ))
) -> glint:command(AJNQ).
