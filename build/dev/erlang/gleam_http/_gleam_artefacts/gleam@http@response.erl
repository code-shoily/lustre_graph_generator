...
-type response(GCA) :: {response, integer(), list({binary(), binary()}), GCA}.
...
-spec set_header(response(GCP), binary(), binary()) -> response(GCP).
...
-spec prepend_header(response(GCS), binary(), binary()) -> response(GCS).
...
-spec set_body(response(any()), GCX) -> response(GCX).
...
-spec try_map(response(GCB), fun((GCB) -> {ok, GCD} | {error, GCE})) -> {ok,
        response(GCD)} |
    {error, GCE}.
...
-spec map(response(GCZ), fun((GCZ) -> GDB)) -> response(GDB).
...
-spec expire_cookie(response(GDK), binary(), gleam@http@cookie:attributes()) -> response(GDK).
