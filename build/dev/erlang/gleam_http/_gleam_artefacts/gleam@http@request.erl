...
-spec set_header(request(FSG), binary(), binary()) -> request(FSG).
...
-spec prepend_header(request(FSJ), binary(), binary()) -> request(FSJ).
...
-spec set_body(request(any()), FSO) -> request(FSO).
set_body(Req, Body) ->
    {request,
...
-spec map(request(FSQ), fun((FSQ) -> FSS)) -> request(FSS).
...
-spec set_query(request(FTC), list({binary(), binary()})) -> request(FTC).
set_query(Req, Query) ->
    Query@1 = begin
...
-spec set_method(request(FTG), gleam@http:method()) -> request(FTG).
...
-spec set_scheme(request(FTN), gleam@http:scheme()) -> request(FTN).
...
-spec set_host(request(FTQ), binary()) -> request(FTQ).
set_host(Req, Host) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_port(request(FTT), integer()) -> request(FTT).
set_port(Req, Port) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_path(request(FTW), binary()) -> request(FTW).
set_path(Req, Path) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_cookie(request(FTZ), binary(), binary()) -> request(FTZ).
...
-spec remove_cookie(request(FUF), binary()) -> request(FUF).
