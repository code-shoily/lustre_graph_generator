...
-type grid(ANPP, ANPQ) :: {grid,
        yog@model:graph(ANPP, ANPQ),
        integer(),
        integer()}.
...
-spec get_cell(grid(ANPW, any()), integer(), integer()) -> {ok, ANPW} |
    {error, nil}.
...
-spec to_graph(grid(ANQC, ANQD)) -> yog@model:graph(ANQC, ANQD).
to_graph(Grid) ->
...
-spec find_node(grid(ANQI, any()), fun((ANQI) -> boolean())) -> {ok, integer()} |
    {error, nil}.
