...
-spec transpose(yog@model:graph(AOJU, AOJV)) -> yog@model:graph(AOJU, AOJV).
transpose(Graph) ->
    {graph,
...
-spec map_nodes(yog@model:graph(AOKA, AOKB), fun((AOKA) -> AOKE)) -> yog@model:graph(AOKE, AOKB).
map_nodes(Graph, Fun) ->
...
-spec map_edges(yog@model:graph(AOKH, AOKI), fun((AOKI) -> AOKL)) -> yog@model:graph(AOKH, AOKL).
map_edges(Graph, Fun) ->
...
-spec filter_nodes(yog@model:graph(AOKO, AOKP), fun((AOKO) -> boolean())) -> yog@model:graph(AOKO, AOKP).
...
-spec merge(yog@model:graph(AOKU, AOKV), yog@model:graph(AOKU, AOKV)) -> yog@model:graph(AOKU, AOKV).
merge(Base, Other) ->
...
-spec subgraph(yog@model:graph(AOLC, AOLD), list(integer())) -> yog@model:graph(AOLC, AOLD).
subgraph(Graph, Ids) ->
