...
-type path(ARDN) :: {path, list(integer()), ARDN}.

-type bellman_ford_result(ARDO) :: {shortest_path, path(ARDO)} |
    negative_cycle |
    no_path.

-type implicit_bellman_ford_result(ARDP) :: {found_goal, ARDP} |
...
-spec compare_frontier(
    {AREG, list(integer())},
    {AREG, list(integer())},
    fun((AREG, AREG) -> gleam@order:order())
) -> gleam@order:order().
...
-spec compare_distance_frontier(
    {AREJ, integer()},
    {AREJ, integer()},
    fun((AREJ, AREJ) -> gleam@order:order())
) -> gleam@order:order().
...
-spec compare_a_star_frontier(
    {AREK, AREK, list(integer())},
    {AREK, AREK, list(integer())},
    fun((AREK, AREK) -> gleam@order:order())
) -> gleam@order:order().
...
-spec should_explore_node(
    gleam@dict:dict(integer(), AREN),
    integer(),
    AREN,
    fun((AREN, AREN) -> gleam@order:order())
) -> boolean().
...
-spec do_dijkstra(
    yog@model:graph(any(), ARDX),
    integer(),
    gleamy@pairing_heap:heap({ARDX, list(integer())}),
    gleam@dict:dict(integer(), ARDX),
    fun((ARDX, ARDX) -> ARDX),
    fun((ARDX, ARDX) -> gleam@order:order())
) -> gleam@option:option(path(ARDX)).
...
-spec shortest_path(
    yog@model:graph(any(), ARDR),
    integer(),
    integer(),
    ARDR,
    fun((ARDR, ARDR) -> ARDR),
    fun((ARDR, ARDR) -> gleam@order:order())
) -> gleam@option:option(path(ARDR)).
...
-spec do_single_source_dijkstra(
    yog@model:graph(any(), AREX),
    gleamy@pairing_heap:heap({AREX, integer()}),
    gleam@dict:dict(integer(), AREX),
    fun((AREX, AREX) -> AREX),
    fun((AREX, AREX) -> gleam@order:order())
) -> gleam@dict:dict(integer(), AREX).
...
-spec single_source_distances(
    yog@model:graph(any(), ARER),
    integer(),
    ARER,
    fun((ARER, ARER) -> ARER),
    fun((ARER, ARER) -> gleam@order:order())
) -> gleam@dict:dict(integer(), ARER).
...
-spec do_a_star(
    yog@model:graph(any(), AROR),
    integer(),
    gleamy@pairing_heap:heap({AROO, AROO, list(integer())}),
    gleam@dict:dict(integer(), AROO),
    fun((AROO, AROR) -> AROO),
    fun((AROO, AROO) -> gleam@order:order()),
    fun((integer(), integer()) -> AROR)
) -> gleam@option:option(path(AROO)).
...
-spec a_star(
    yog@model:graph(any(), ARFG),
    integer(),
    integer(),
    ARFG,
    fun((ARFG, ARFG) -> ARFG),
    fun((ARFG, ARFG) -> gleam@order:order()),
    fun((integer(), integer()) -> ARFG)
) -> gleam@option:option(path(ARFG)).
...
-spec relaxation_passes(
    yog@model:graph(any(), ARFZ),
    list(integer()),
    gleam@dict:dict(integer(), ARFZ),
    gleam@dict:dict(integer(), integer()),
    integer(),
    fun((ARFZ, ARFZ) -> ARFZ),
    fun((ARFZ, ARFZ) -> gleam@order:order())
) -> {gleam@dict:dict(integer(), ARFZ), gleam@dict:dict(integer(), integer())}.
relaxation_passes(
    Graph,
    Nodes,
    Distances,
...
-spec has_negative_cycle(
    yog@model:graph(any(), ARGM),
    list(integer()),
    gleam@dict:dict(integer(), ARGM),
    fun((ARGM, ARGM) -> ARGM),
    fun((ARGM, ARGM) -> gleam@order:order())
) -> boolean().
...
-spec bellman_ford(
    yog@model:graph(any(), ARFU),
    integer(),
    integer(),
    ARFU,
    fun((ARFU, ARFU) -> ARFU),
    fun((ARFU, ARFU) -> gleam@order:order())
) -> bellman_ford_result(ARFU).
...
-spec detect_negative_cycle(
    gleam@dict:dict({integer(), integer()}, ARHG),
    list(integer()),
    ARHG,
    fun((ARHG, ARHG) -> gleam@order:order())
) -> boolean().
...
-spec floyd_warshall(
    yog@model:graph(any(), ARGZ),
    ARGZ,
    fun((ARGZ, ARGZ) -> ARGZ),
    fun((ARGZ, ARGZ) -> gleam@order:order())
) -> {ok, gleam@dict:dict({integer(), integer()}, ARGZ)} | {error, nil}.
...
-spec distance_matrix(
    yog@model:graph(any(), ARHL),
    list(integer()),
    ARHL,
    fun((ARHL, ARHL) -> ARHL),
    fun((ARHL, ARHL) -> gleam@order:order())
) -> {ok, gleam@dict:dict({integer(), integer()}, ARHL)} | {error, nil}.
...
-spec do_implicit_dijkstra(
    gleamy@pairing_heap:heap({ARHX, ARHY}),
    gleam@dict:dict(ARHY, ARHX),
    fun((ARHY) -> list({ARHY, ARHX})),
    fun((ARHY) -> boolean()),
    fun((ARHX, ARHX) -> ARHX),
    fun((ARHX, ARHX) -> gleam@order:order())
) -> gleam@option:option(ARHX).
...
-spec implicit_dijkstra(
    ARHT,
    fun((ARHT) -> list({ARHT, ARHU})),
    fun((ARHT) -> boolean()),
    ARHU,
    fun((ARHU, ARHU) -> ARHU),
    fun((ARHU, ARHU) -> gleam@order:order())
) -> gleam@option:option(ARHU).
...
-spec do_implicit_dijkstra_by(
    gleamy@pairing_heap:heap({ARIJ, ARIK}),
    gleam@dict:dict(ARIM, ARIJ),
    fun((ARIK) -> list({ARIK, ARIJ})),
    fun((ARIK) -> ARIM),
    fun((ARIK) -> boolean()),
    fun((ARIJ, ARIJ) -> ARIJ),
    fun((ARIJ, ARIJ) -> gleam@order:order())
) -> gleam@option:option(ARIJ).
...
-spec implicit_dijkstra_by(
    ARIE,
    fun((ARIE) -> list({ARIE, ARIF})),
    fun((ARIE) -> any()),
    fun((ARIE) -> boolean()),
    ARIF,
    fun((ARIF, ARIF) -> ARIF),
    fun((ARIF, ARIF) -> gleam@order:order())
) -> gleam@option:option(ARIF).
...
-spec do_implicit_a_star(
    gleamy@pairing_heap:heap({ARIV, ARIV, ARIW}),
    gleam@dict:dict(ARIW, ARIV),
    fun((ARIW) -> list({ARIW, ARIV})),
    fun((ARIW) -> boolean()),
    fun((ARIW) -> ARIV),
    fun((ARIV, ARIV) -> ARIV),
    fun((ARIV, ARIV) -> gleam@order:order())
) -> gleam@option:option(ARIV).
...
-spec implicit_a_star(
    ARIR,
    fun((ARIR) -> list({ARIR, ARIS})),
    fun((ARIR) -> boolean()),
    fun((ARIR) -> ARIS),
    ARIS,
    fun((ARIS, ARIS) -> ARIS),
    fun((ARIS, ARIS) -> gleam@order:order())
) -> gleam@option:option(ARIS).
...
-spec do_implicit_a_star_by(
    gleamy@pairing_heap:heap({ARJH, ARJH, ARJI}),
    gleam@dict:dict(ARJK, ARJH),
    fun((ARJI) -> list({ARJI, ARJH})),
    fun((ARJI) -> ARJK),
    fun((ARJI) -> boolean()),
    fun((ARJI) -> ARJH),
    fun((ARJH, ARJH) -> ARJH),
    fun((ARJH, ARJH) -> gleam@order:order())
) -> gleam@option:option(ARJH).
...
-spec implicit_a_star_by(
    ARJC,
    fun((ARJC) -> list({ARJC, ARJD})),
    fun((ARJC) -> any()),
    fun((ARJC) -> boolean()),
    fun((ARJC) -> ARJD),
    ARJD,
    fun((ARJD, ARJD) -> ARJD),
    fun((ARJD, ARJD) -> gleam@order:order())
) -> gleam@option:option(ARJD).
...
-spec do_implicit_bellman_ford(
    yog@internal@queue:queue(ARJT),
    gleam@dict:dict(ARJT, ARJV),
    gleam@dict:dict(ARJT, integer()),
    gleam@set:set(ARJT),
    fun((ARJT) -> list({ARJT, ARJV})),
    fun((ARJT) -> boolean()),
    ARJV,
    fun((ARJV, ARJV) -> ARJV),
    fun((ARJV, ARJV) -> gleam@order:order())
) -> implicit_bellman_ford_result(ARJV).
...
-spec implicit_bellman_ford(
    ARJP,
    fun((ARJP) -> list({ARJP, ARJQ})),
    fun((ARJP) -> boolean()),
    ARJQ,
    fun((ARJQ, ARJQ) -> ARJQ),
    fun((ARJQ, ARJQ) -> gleam@order:order())
) -> implicit_bellman_ford_result(ARJQ).
...
-spec do_implicit_bellman_ford_by(
    yog@internal@queue:queue(ARKI),
    gleam@dict:dict(ARKK, {ARKL, ARKI}),
    gleam@dict:dict(ARKK, integer()),
    gleam@set:set(ARKI),
    fun((ARKI) -> list({ARKI, ARKL})),
    fun((ARKI) -> ARKK),
    fun((ARKI) -> boolean()),
    ARKL,
    fun((ARKL, ARKL) -> ARKL),
    fun((ARKL, ARKL) -> gleam@order:order())
) -> implicit_bellman_ford_result(ARKL).
...
-spec implicit_bellman_ford_by(
    ARKD,
    fun((ARKD) -> list({ARKD, ARKE})),
    fun((ARKD) -> any()),
    fun((ARKD) -> boolean()),
    ARKE,
    fun((ARKE, ARKE) -> ARKE),
    fun((ARKE, ARKE) -> gleam@order:order())
) -> implicit_bellman_ford_result(ARKE).
