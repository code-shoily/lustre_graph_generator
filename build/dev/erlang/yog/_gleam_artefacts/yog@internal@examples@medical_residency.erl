...
?DOC(false).
-spec list_at(list(ATMM), integer()) -> gleam@option:option(ATMM).
list_at(Lst, Index) ->
    case {Index, Lst} of
        {0, [First | _]} ->
            {some, First};
...
?DOC(false).
-spec do_index_of(list(ATMS), ATMS, integer()) -> gleam@option:option(integer()).
...
?DOC(false).
-spec list_index_of(list(ATMP), ATMP) -> gleam@option:option(integer()).
