...
-type disjoint_set(APCM) :: {disjoint_set,
        gleam@dict:dict(APCM, APCM),
        gleam@dict:dict(APCM, integer())}.
...
-spec add(disjoint_set(APCP), APCP) -> disjoint_set(APCP).
...
-spec find(disjoint_set(APCS), APCS) -> {disjoint_set(APCS), APCS}.
...
-spec union(disjoint_set(APCV), APCV, APCV) -> disjoint_set(APCV).
...
-spec from_pairs(list({APCY, APCY})) -> disjoint_set(APCY).
from_pairs(Pairs) ->
    gleam@list:fold(
        Pairs,
        new(),
...
-spec connected(disjoint_set(APDB), APDB, APDB) -> {disjoint_set(APDB),
    boolean()}.
connected(Dsu, X, Y) ->
...
-spec find_root_readonly(disjoint_set(APDM), APDM) -> APDM.
...
-spec to_lists(disjoint_set(APDI)) -> list(list(APDI)).
to_lists(Dsu) ->
    _pipe = maps:keys(erlang:element(2, Dsu)),
