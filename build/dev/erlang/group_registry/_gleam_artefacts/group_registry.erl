...
-type group_registry(QJZ) :: any() | {gleam_phantom, QJZ}.

-type message(QKA) :: any() | {gleam_phantom, QKA}.

-type do_not_leak() :: any().

-file("src/group_registry.gleam", 104).
-spec get_registry(gleam@erlang@process:name(message(QKL))) -> group_registry(QKL).
get_registry(Name) ->
...
-spec start(gleam@erlang@process:name(message(QKB))) -> {ok,
        gleam@otp@actor:started(group_registry(QKB))} |
...
-spec supervised(gleam@erlang@process:name(message(QKG))) -> gleam@otp@supervision:child_specification(group_registry(QKG)).
supervised(Name) ->
...
-spec make_subject(group_registry(QKZ), binary(), gleam@erlang@process:pid_()) -> gleam@erlang@process:subject(QKZ).
...
-spec join(group_registry(QKP), binary(), gleam@erlang@process:pid_()) -> gleam@erlang@process:subject(QKP).
...
-spec members(group_registry(QKV), binary()) -> list(gleam@erlang@process:subject(QKV)).
