...
-spec map({ok, COC} | {error, COD}, fun((COC) -> COG)) -> {ok, COG} |
    {error, COD}.
map(Result, Fun) ->
    case Result of
        {ok, X} ->
...
-spec map_error({ok, COJ} | {error, COK}, fun((COK) -> CON)) -> {ok, COJ} |
    {error, CON}.
...
-spec flatten({ok, {ok, COQ} | {error, COR}} | {error, COR}) -> {ok, COQ} |
    {error, COR}.
flatten(Result) ->
    case Result of
        {ok, X} ->
...
-spec 'try'({ok, COY} | {error, COZ}, fun((COY) -> {ok, CPC} | {error, COZ})) -> {ok,
        CPC} |
    {error, COZ}.
'try'(Result, Fun) ->
    case Result of
...
-spec unwrap({ok, CPH} | {error, any()}, CPH) -> CPH.
...
-spec lazy_unwrap({ok, CPL} | {error, any()}, fun(() -> CPL)) -> CPL.
...
-spec unwrap_error({ok, any()} | {error, CPQ}, CPQ) -> CPQ.
...
-spec 'or'({ok, CPT} | {error, CPU}, {ok, CPT} | {error, CPU}) -> {ok, CPT} |
    {error, CPU}.
'or'(First, Second) ->
    case First of
        {ok, _} ->
...
-spec lazy_or({ok, CQB} | {error, CQC}, fun(() -> {ok, CQB} | {error, CQC})) -> {ok,
        CQB} |
    {error, CQC}.
...
-spec all(list({ok, CQJ} | {error, CQK})) -> {ok, list(CQJ)} | {error, CQK}.
all(Results) ->
    gleam@list:try_map(Results, fun(Result) -> Result end).

-file("src/gleam/result.gleam", 337).
-spec partition_loop(list({ok, CQY} | {error, CQZ}), list(CQY), list(CQZ)) -> {list(CQY),
    list(CQZ)}.
...
-spec partition(list({ok, CQR} | {error, CQS})) -> {list(CQR), list(CQS)}.
partition(Results) ->
...
-spec replace({ok, any()} | {error, CRH}, CRK) -> {ok, CRK} | {error, CRH}.
...
-spec replace_error({ok, CRN} | {error, any()}, CRR) -> {ok, CRN} | {error, CRR}.
...
-spec values(list({ok, CRU} | {error, any()})) -> list(CRU).
values(Results) ->
