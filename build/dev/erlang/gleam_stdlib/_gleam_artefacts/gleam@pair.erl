...
-spec map_first({CNL, CNM}, fun((CNL) -> CNN)) -> {CNN, CNM}.
map_first(Pair, Fun) ->
    {A, B} = Pair,
    {Fun(A), B}.
...
-spec map_second({CNO, CNP}, fun((CNP) -> CNQ)) -> {CNO, CNQ}.
map_second(Pair, Fun) ->
    {A, B} = Pair,
    {A, Fun(B)}.
