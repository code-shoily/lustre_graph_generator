...
-opaque set(CWR) :: {set, gleam@dict:dict(CWR, list(nil))}.
...
-spec contains(set(CXC), CXC) -> boolean().
contains(Set, Member) ->
...
-spec delete(set(CXE), CXE) -> set(CXE).
delete(Set, Member) ->
...
-spec fold(set(CXN), CXP, fun((CXP, CXN) -> CXP)) -> CXP.
...
-spec filter(set(CXQ), fun((CXQ) -> boolean())) -> set(CXQ).
...
-spec drop(set(CXX), list(CXX)) -> set(CXX).
drop(Set, Disallowed) ->
...
-spec take(set(CYB), list(CYB)) -> set(CYB).
take(Set, Desired) ->
    {set, gleam@dict:take(erlang:element(2, Set), Desired)}.

-file("src/gleam/set.gleam", 289).
-spec order(set(CYJ), set(CYJ)) -> {set(CYJ), set(CYJ)}.
order(First, Second) ->
...
-spec intersection(set(CYO), set(CYO)) -> set(CYO).
...
-spec difference(set(CYS), set(CYS)) -> set(CYS).
...
-spec is_subset(set(CYW), set(CYW)) -> boolean().
...
-spec is_disjoint(set(CYZ), set(CYZ)) -> boolean().
...
-spec each(set(CZG), fun((CZG) -> any())) -> nil.
each(Set, Fun) ->
    fold(
        Set,
        nil,
...
-spec insert(set(CWZ), CWZ) -> set(CWZ).
insert(Set, Member) ->
...
-spec from_list(list(CXK)) -> set(CXK).
from_list(Members) ->
...
-spec map(set(CXT), fun((CXT) -> CXV)) -> set(CXV).
map(Set, Fun) ->
...
-spec union(set(CYF), set(CYF)) -> set(CYF).
union(First, Second) ->
    {Larger, Smaller} = order(First, Second),
...
-spec symmetric_difference(set(CZC), set(CZC)) -> set(CZC).
