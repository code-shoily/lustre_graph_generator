...
-type child_specification(HFW) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(HFW)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(HFX)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(HFX).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(HGC)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(HGC).
supervisor(Start) ->
...
-spec significant(child_specification(HGH), boolean()) -> child_specification(HGH).
...
-spec timeout(child_specification(HGK), integer()) -> child_specification(HGK).
timeout(Child, Ms) ->
...
-spec restart(child_specification(HGN), restart()) -> child_specification(HGN).
...
-spec map_data(child_specification(HGQ), fun((HGQ) -> HGS)) -> child_specification(HGS).
