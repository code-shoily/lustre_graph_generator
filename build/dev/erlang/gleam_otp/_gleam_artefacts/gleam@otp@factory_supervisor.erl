...
-opaque supervisor(HIK, HIL) :: {supervisor, gleam@erlang@process:pid_()} |
    {named_supervisor, gleam@erlang@process:name(message(HIK, HIL))}.

-type message(HIM, HIN) :: any() | {gleam_phantom, HIM, HIN}.

-opaque builder(HIO, HIP) :: {builder,
        gleam@otp@supervision:child_type(),
        fun((HIO) -> {ok, gleam@otp@actor:started(HIP)} |
            {error, gleam@otp@actor:start_error()}),
        gleam@otp@supervision:restart(),
        integer(),
        integer(),
        gleam@option:option(gleam@erlang@process:name(message(HIO, HIP)))}.

-type erlang_start_flags() :: any().

-type erlang_supervisor_name(HIQ, HIR) :: {local,
        gleam@erlang@process:name(message(HIQ, HIR))}.

-type strategy() :: simple_one_for_one.

-type erlang_start_flag(HIS) :: {strategy, strategy()} |
    {intensity, integer()} |
    {period, integer()} |
    {gleam_phantom, HIS}.
...
-type erlang_child_spec_property(HIT, HIU) :: {id, integer()} |
    {start,
        {gleam@erlang@atom:atom_(),
            gleam@erlang@atom:atom_(),
            list(fun((HIT) -> {ok, gleam@otp@actor:started(HIU)} |
                {error, gleam@otp@actor:start_error()}))}} |
...
-spec get_by_name(gleam@erlang@process:name(message(HIV, HIW))) -> supervisor(HIV, HIW).
get_by_name(Name) ->
...
-spec named(builder(HJM, HJN), gleam@erlang@process:name(message(HJM, HJN))) -> builder(HJM, HJN).
named(Builder, Name) ->
    {builder,
...
-spec restart_tolerance(builder(HJV, HJW), integer(), integer()) -> builder(HJV, HJW).
...
-spec timeout(builder(HKB, HKC), integer()) -> builder(HKB, HKC).
timeout(Builder, Ms) ->
...
-spec restart_strategy(builder(HKH, HKI), gleam@otp@supervision:restart()) -> builder(HKH, HKI).
...
-spec start(builder(HKN, HKO)) -> {ok,
        gleam@otp@actor:started(supervisor(HKN, HKO))} |
    {error, gleam@otp@actor:start_error()}.
start(Builder) ->
...
-spec supervised(builder(HLM, HLN)) -> gleam@otp@supervision:child_specification(supervisor(HLM, HLN)).
supervised(Builder) ->
...
-spec start_child(supervisor(HLT, HLU), HLT) -> {ok,
        gleam@otp@actor:started(HLU)} |
    {error, gleam@otp@actor:start_error()}.
...
-spec start_child_callback(
    fun((HMR) -> {ok, gleam@otp@actor:started(HMS)} |
        {error, gleam@otp@actor:start_error()}),
    HMR
) -> gleam@otp@internal@result2:result2(gleam@erlang@process:pid_(), HMS, gleam@otp@actor:start_error()).
...
-spec worker_child(
    fun((HJC) -> {ok, gleam@otp@actor:started(HJD)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(HJC, HJD).
...
-spec supervisor_child(
    fun((HJH) -> {ok, gleam@otp@actor:started(HJI)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(HJH, HJI).
