...
-type message(GMR) :: {message, GMR} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(GMS, GMT) :: {continue,
        GMS,
        gleam@option:option(gleam@erlang@process:selector(GMT))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(GMU, GMV) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        GMU,
        gleam@erlang@process:selector(message(GMV)),
        gleam@otp@system:debug_state(),
        fun((GMU, GMV) -> next(GMU, GMV))}.

-type started(GMW) :: {started, gleam@erlang@process:pid_(), GMW}.

-opaque initialised(GMX, GMY, GMZ) :: {initialised,
        GMX,
        gleam@option:option(gleam@erlang@process:selector(GMY)),
        GMZ}.

-opaque builder(GNA, GNB, GNC) :: {builder,
        fun((gleam@erlang@process:subject(GNB)) -> {ok,
                initialised(GNA, GNB, GNC)} |
            {error, binary()}),
        integer(),
        fun((GNA, GNB) -> next(GNA, GNB)),
        gleam@option:option(gleam@erlang@process:name(GNB))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(GND) :: {ack, {ok, GND} | {error, binary()}} |
...
-spec continue(GNI) -> next(GNI, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(GNU, GNV), gleam@erlang@process:selector(GNV)) -> next(GNU, GNV).
...
-spec initialised(GOB) -> initialised(GOB, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(GOG, any(), GOI),
    gleam@erlang@process:selector(GOM)
) -> initialised(GOG, GOM, GOI).
...
-spec returning(initialised(GOR, GOS, any()), GOX) -> initialised(GOR, GOS, GOX).
...
-spec new(GPB) -> builder(GPB, GPC, gleam@erlang@process:subject(GPC)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(GPH)) -> {ok, initialised(GPJ, GPH, GPK)} |
        {error, binary()})
) -> builder(GPJ, GPH, GPK).
...
-spec on_message(builder(GPT, GPU, GPV), fun((GPT, GPU) -> next(GPT, GPU))) -> builder(GPT, GPU, GPV).
...
-spec named(builder(GQE, GQF, GQG), gleam@erlang@process:name(GQF)) -> builder(GQE, GQF, GQG).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(GQT))) -> gleam@erlang@process:selector(message(GQT)).
...
-spec receive_message(self(any(), GQP)) -> message(GQP).
...
-spec initialise_actor(
    builder(any(), any(), GRM),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, GRM} | {error, binary()})
...
-spec start(builder(any(), any(), GRZ)) -> {ok, started(GRZ)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(GSG), GSG) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(GSI),
    integer(),
    fun((gleam@erlang@process:subject(GSK)) -> GSI)
) -> GSK.
