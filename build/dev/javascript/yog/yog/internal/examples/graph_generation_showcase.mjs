/// <reference types="./graph_generation_showcase.d.mts" />
import * as $int from "../../../../gleam_stdlib/gleam/int.mjs";
import * as $io from "../../../../gleam_stdlib/gleam/io.mjs";
import { Empty as $Empty } from "../../../gleam.mjs";
import * as $generators from "../../../yog/generators.mjs";
...
function list_length(list) {
  if (list instanceof $Empty) {
    ...
    let rest = list.tail;
    return 1 + list_length(rest);
  }
}

function list_fold(loop$list, loop$initial, loop$fun) {
  while (true) {
    let list = loop$list;
...
function count_edges(graph) {
  ...
  return list_fold(
    ...
    (count, node) => {
      ...
      return count + list_length(successors);
    },
  );
}

function print_graph_stats(_, graph) {
  let _block;
  let _pipe = $model.all_nodes(graph);
  _block = list_length(_pipe);
  let node_count = _block;
  ...
  if ($ instanceof $model.Directed) {
    ...
    _block$1 = globalThis.Math.trunc(edge_count / 2);
  }
  let display_edges = _block$1;
  $io.println("  Nodes: " + $int.to_string(node_count));
  return $io.println("  Edges: " + $int.to_string(display_edges));
}
...
export function main() {
  ...
  let directed_k4 = $generators.complete_with_type(4, new $model.Directed());
  let undirected_k4 = $generators.complete_with_type(4, new $model.Undirected());
  $io.println("Directed K_4 edges: " + $int.to_string(count_edges(directed_k4)));
  $io.println(
    "Undirected K_4 edges: " + $int.to_string(
      globalThis.Math.trunc(count_edges(undirected_k4) / 2),
    ),
  );
  return $io.println("(Directed has edges in both directions)");
}
