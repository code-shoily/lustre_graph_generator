...
-type walk_metadata(JDZ) :: {walk_metadata, integer(), gleam@option:option(JDZ)}.
...
-spec do_fold_walk_bfs(
    ...
    JFG,
    fun((JFG, integer(), walk_metadata(integer())) -> {walk_control(), JFG})
) -> JFG.
...
-spec do_fold_walk_dfs(
    ...
    gleam@set:set(integer()),
    JFP,
    fun((JFP, integer(), walk_metadata(integer())) -> {walk_control(), JFP})
) -> JFP.
...
-spec fold_walk(
    ...
    order(),
    JEO,
    fun((JEO, integer(), walk_metadata(integer())) -> {walk_control(), JEO})
) -> JEO.
...
-spec do_virtual_bfs(
    yog@internal@queue:queue({JFR, walk_metadata(JFR)}),
    gleam@set:set(JFR),
    JFV,
    fun((JFR) -> list(JFR)),
    fun((JFV, JFR, walk_metadata(JFR)) -> {walk_control(), JFV})
) -> JFV.
...
-spec do_virtual_dfs(
    list({JFY, walk_metadata(JFY)}),
    gleam@set:set(JFY),
    JGC,
    fun((JFY) -> list(JFY)),
    fun((JGC, JFY, walk_metadata(JFY)) -> {walk_control(), JGC})
) -> JGC.
...
-spec implicit_fold(
    ...
    JER,
    fun((JEQ) -> list(JEQ)),
    fun((JER, JEQ, walk_metadata(JEQ)) -> {walk_control(), JER})
) -> JER.
...
-spec do_virtual_bfs_by(
    yog@internal@queue:queue({JGF, walk_metadata(JGF)}),
    gleam@set:set(JGI),
    JGK,
    fun((JGF) -> list(JGF)),
    fun((JGF) -> JGI),
    fun((JGK, JGF, walk_metadata(JGF)) -> {walk_control(), JGK})
) -> JGK.
...
-spec do_virtual_dfs_by(
    list({JGN, walk_metadata(JGN)}),
    gleam@set:set(JGQ),
    JGS,
    fun((JGN) -> list(JGN)),
    fun((JGN) -> JGQ),
    fun((JGS, JGN, walk_metadata(JGN)) -> {walk_control(), JGS})
) -> JGS.
...
-spec implicit_fold_by(
    ...
    fun((JEU) -> list(JEU)),
    fun((JEU) -> any()),
    fun((JEV, JEU, walk_metadata(JEU)) -> {walk_control(), JEV})
) -> JEV.
...
-spec do_implicit_dijkstra(
    gleamy@pairing_heap:heap({integer(), JGY}),
    gleam@dict:dict(JGY, integer()),
    JHC,
    fun((JGY) -> list({JGY, integer()})),
    fun((JHC, JGY, integer()) -> {walk_control(), JHC})
) -> JHC.
...
-spec implicit_dijkstra(
    JGV,
    JGW,
    fun((JGV) -> list({JGV, integer()})),
    fun((JGW, JGV, integer()) -> {walk_control(), JGW})
) -> JGW.
