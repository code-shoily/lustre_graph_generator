...
-type builder(HOE, HOF) :: {builder,
        yog@model:graph(HOE, HOF),
        gleam@dict:dict(HOE, integer()),
        integer()}.
...
-spec ensure_node(builder(HOS, HOT), HOS) -> {builder(HOS, HOT), integer()}.
...
-spec add_node(builder(HOY, HOZ), HOY) -> builder(HOY, HOZ).
...
-spec add_edge(builder(HPE, HPF), HPE, HPE, HPF) -> builder(HPE, HPF).
...
-spec add_unweighted_edge(builder(HPK, nil), HPK, HPK) -> builder(HPK, nil).
...
-spec add_simple_edge(builder(HPP, integer()), HPP, HPP) -> builder(HPP, integer()).
...
-spec get_id(builder(HPU, any()), HPU) -> {ok, integer()} | {error, nil}.
...
-spec to_graph(builder(HQA, HQB)) -> yog@model:graph(HQA, HQB).
to_graph(Builder) ->
    erlang:element(2, Builder).
...
-spec from_list(yog@model:graph_type(), list({HQG, HQG, HQH})) -> builder(HQG, HQH).
...
-spec from_unweighted_list(yog@model:graph_type(), list({HQL, HQL})) -> builder(HQL, nil).
...
-spec all_labels(builder(HQP, any())) -> list(HQP).
all_labels(Builder) ->
    maps:keys(erlang:element(3, Builder)).

-file("src/yog/builder/labeled.gleam", 379).
-spec list_map_ids_to_labels(list({integer(), HRI}), yog@model:graph(HRK, HRI)) -> list({HRK,
    HRI}).
...
-spec successors(builder(HQU, HQV), HQU) -> {ok, list({HQU, HQV})} |
    {error, nil}.
...
-spec predecessors(builder(HRB, HRC), HRB) -> {ok, list({HRB, HRC})} |
    {error, nil}.
