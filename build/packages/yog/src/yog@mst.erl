...
-type edge(NKD) :: {edge, integer(), integer(), NKD}.
...
-spec kruskal(
    yog@model:graph(any(), NKF),
    fun((NKF, NKF) -> gleam@order:order())
) -> list(edge(NKF)).
...
-spec get_edges_from_node(yog@model:graph(any(), NLJ), integer()) -> list(edge(NLJ)).
...
-spec do_prim(
    yog@model:graph(any(), NKY),
    gleamy@pairing_heap:heap(edge(NKY)),
...
-spec prim(yog@model:graph(any(), NKS), fun((NKS, NKS) -> gleam@order:order())) -> list(edge(NKS)).
prim(Graph, Compare) ->
    Node_ids = maps:keys(erlang:element(3, Graph)),
    case Node_ids of
        [] ->
