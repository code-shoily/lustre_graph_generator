...
-type max_flow_result(MTB) :: {max_flow_result,
        MTB,
        yog@model:graph(nil, MTB),
        integer(),
        integer()}.
...
-spec do_reconstruct_path(
    gleam@dict:dict(integer(), {integer(), MUR}),
    integer(),
    list(integer()),
    MUR,
    boolean(),
...
-spec reconstruct_path(
    gleam@dict:dict(integer(), {integer(), MUN}),
    integer(),
    MUN,
    fun((MUN, MUN) -> MUN)
) -> {list(integer()), MUN}.
...
-spec do_bfs(
    gleam@dict:dict({integer(), integer()}, MUF),
    gleam@dict:dict(integer(), list(integer())),
    yog@internal@queue:queue(integer()),
    gleam@dict:dict(integer(), {integer(), MUF}),
    integer(),
    MUF,
    fun((MUF, MUF) -> gleam@order:order()),
    fun((MUF, MUF) -> MUF)
) -> {ok, {list(integer()), MUF}} | {error, nil}.
...
-spec find_augmenting_path_bfs(
    gleam@dict:dict({integer(), integer()}, MUA),
    gleam@dict:dict(integer(), list(integer())),
    integer(),
    integer(),
    MUA,
    fun((MUA, MUA) -> gleam@order:order()),
    fun((MUA, MUA) -> MUA)
) -> {ok, {list(integer()), MUA}} | {error, nil}.
...
-spec do_augment_path(
    gleam@dict:dict({integer(), integer()}, MVA),
    list(integer()),
    MVA,
    fun((MVA, MVA) -> MVA),
    fun((MVA, MVA) -> MVA)
...
-spec augment_path(
    gleam@dict:dict({integer(), integer()}, MUW),
    list(integer()),
    MUW,
    fun((MUW, MUW) -> MUW),
    fun((MUW, MUW) -> MUW)
...
-spec ford_fulkerson(
    gleam@dict:dict({integer(), integer()}, MTX),
    gleam@dict:dict(integer(), list(integer())),
    integer(),
    integer(),
    MTX,
    ...
    fun((MTX, MTX) -> MTX),
    fun((MTX, MTX) -> MTX),
    fun((MTX, MTX) -> gleam@order:order()),
    fun((MTX, MTX) -> MTX)
) -> {gleam@dict:dict({integer(), integer()}, MTX), MTX}.
ford_fulkerson(
    Residuals,
    Adj_list,
    Source,
...
-spec residuals_to_graph(
    gleam@dict:dict({integer(), integer()}, MVE),
    gleam@set:set(integer())
) -> yog@model:graph(nil, MVE).
...
-spec build_residuals(yog@model:graph(any(), MTQ), MTQ) -> {gleam@dict:dict({integer(),
        integer()}, MTQ),
    gleam@set:set(integer())}.
...
-spec edmonds_karp(
    yog@model:graph(any(), MTJ),
    integer(),
    integer(),
    MTJ,
    fun((MTJ, MTJ) -> MTJ),
    fun((MTJ, MTJ) -> MTJ),
    fun((MTJ, MTJ) -> gleam@order:order()),
    fun((MTJ, MTJ) -> MTJ)
) -> max_flow_result(MTJ).
...
-spec do_dfs_reachable(
    yog@model:graph(nil, MVS),
    list(integer()),
    gleam@set:set(integer()),
    MVS,
    fun((MVS, MVS) -> gleam@order:order())
) -> gleam@set:set(integer()).
...
-spec find_reachable_nodes(
    yog@model:graph(nil, MVO),
    integer(),
    MVO,
    fun((MVO, MVO) -> gleam@order:order())
) -> gleam@set:set(integer()).
...
-spec min_cut(max_flow_result(MTN), MTN, fun((MTN, MTN) -> gleam@order:order())) -> min_cut().
